...
Despite rapid progress in ASR, the comparative performance of different model families for formal Japanese transcription remains unclear. Many studies have explored various architectures, including traditional GMM–HMM systems \parencite{KaldiCSJResults2015}, hybrid neural models like CRDNN–CTC \parencite{chen2020streaming}, and large-scale transformer models such as Whisper \parencite{radford2023robust}. However, most of the studies focus on English or multilingual datasets, with relatively fewer investigations dedicated to Japanese \parencite{ando2021, xu2023recent}. From that number, only a few is comparing models from different model family. To get the clear comparision between these model families is also challenging as different studies is using different dataset and preproccessing step. As the result, it is difficult to draw definitive conclusions about which model type performs best for formal Japanese transcription tasks. 
...
\begin{enumerate}
    ...
    \item To analyze speech-to-text models to determine the most effective pre-processing and training setup to reduce CER, WER and RTF in Japanese language processing.

    \item To evaluate the CER, WER and the transcription latency using RTF of different speech-to-text model when transcribing Japanese formal and informal language.
\end{enumerate}
...
\begin{enumerate}
    ...
    \item How to calculate the performance and effectiveness using CER, WER and RTF of different speech-to-text model in context of Japanese language?
\end{enumerate}
