...
This section documents the traditional hybrid baseline used in this thesis: a Gaussian Mixture Model -- Hidden Markov Model (GMM--HMM) acoustic model trained and decoded using a Kaldi-style recipe. Although more recent systems such as CRDNN--CTC and Whisper rely on end-to-end neural methods, the GMM--HMM approach is still widely used as a reference point in speech recognition research, especially for controlled experiments on pronunciation, lexicon design, and language modeling. In this work, the GMM--HMM system is trained on the same Japanese lecture-style speech described in Section~\ref{sec:data_collection}, and its output is evaluated with the same scoring protocol described in Section~\ref{sec:text_prep}. The goal is to obtain a strong, fully documented baseline that can be compared directly against the end-to-end systems.

\subsection{Data Preparation for Kaldi}
\label{subsec:gmm_data_prep}

Kaldi expects data in a structured directory format. For each split (train, validation, and test), a data directory is created that explicitly describes the relationship between utterances, audio files, speakers, and transcripts. The core files are \texttt{wav.scp}, \texttt{text}, \texttt{utt2spk}, and \texttt{spk2utt}. The \texttt{wav.scp} file maps each unique utterance identifier to the path of the corresponding audio clip, which has already been standardized to 16~kHz mono WAV as described in Section~\ref{sec:data_collection}. The \texttt{text} file associates the same utterance identifiers with their cleaned, normalized transcripts. The \texttt{utt2spk} file maps each utterance to a speaker label, which may correspond to the talk-level speaker in a TED-style presentation. From \texttt{utt2spk}, Kaldi derives \texttt{spk2utt}, which lists for each speaker all utterances belonging to that speaker.

In parallel with these data directories, a language directory is prepared. This includes a pronunciation lexicon, a list of silence and non-silence phones, and supporting phone-related mappings. The pronunciation lexicon links lexical units in the transcripts to their phone sequences in Japanese. Silence phones and noise markers are defined explicitly so that the model can distinguish between actual spoken content and pauses or background sounds. A language model (LM), typically trained on the cleaned transcripts described in Section~\ref{sec:text_prep}, is compiled alongside the lexicon. The output of this step is a Kaldi-compatible \texttt{lang/} directory containing symbol tables and finite-state transducers (FSTs) that encode how phones map to words and how the language model scores word sequences. Preparing these resources for Japanese requires careful handling of tokenization and pronunciation. Because Japanese does not include whitespace-delimited “words” in the same way as English, a consistent segmentation strategy must be applied so that the resulting units align with both the lexicon and the LM.

Once the data directories and language resources are complete, Kaldi can align audio with text and begin acoustic model training.
