...
This chapter explained the methodology used to evaluate Japanese automatic speech recognition (ASR) across three model families: a traditional Kaldi-style GMM--HMM system, an end-to-end CRDNN--CTC model, and a fine-tuned transformer encoder--decoder model based on Whisper. These models were chosen to represent three major paradigms in ASR: classic hybrid acoustic modeling with an explicit lexicon, recurrent end-to-end modeling with CTC alignment, and large-scale pretrained transformer modeling. All systems were trained and tested on the same curated set of formal Japanese speech. The audio was collected, cleaned, segmented into utterances, normalized, and converted to a consistent 16~kHz mono WAV format. Transcripts were also normalized to produce a publishable-style reference that was used consistently across all models.

The evaluation focused on both accuracy and usability. Accuracy was measured using Character Error Rate (CER) as the primary metric, with Word Error Rate (WER) as supporting context. Usability was measured using Real-Time Factor (RTF), which reflected how fast each model could decode speech relative to the length of the audio. The rest of this chapter described the data pipeline (collection, cleaning, segmentation, and splits), the feature extraction and augmentation strategy, the training and decoding process for each model family, and the scoring protocol used to compute CER, WER, and RTF on the same held-out test set.

\section{Research Design}
This study was organised into four phases: Preparation, Data Collection, Analysis, and Discussion.
An overview of the phases, activities, methods, and expected deliverables was shown in table below.
...
& Defined research area  
  \newline Defined problem statement  
  \newline Defined research objectives, scope, questions, and significance  
...
& Studied prior work on Automatic Speech Recognition (ASR)  
  \newline Identified current challenges in Japanese ASR  
  \newline Identified promising techniques for improving transcription accuracy and latency  
  \newline Collected and prepared speech data  
& Literature review  
...
& Trained and evaluated the selected ASR models  
  \newline Computed evaluation metrics  
  \newline Compared models quantitatively  
  \newline Analysed error patterns  
...
& Interpreted the results  
  \newline Discussed limitations and challenges  
  \newline Related findings to the research objectives  
  \newline Outlined implications and future work  
