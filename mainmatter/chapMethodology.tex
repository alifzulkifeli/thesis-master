...
The RTF will be used to measure the decoding speed of the GMM-HMM system. RTF is defined as the ratio of the time taken to decode an audio segment to the actual duration of that audio segment. An RTF less than 1 indicates that the model can decode faster than real-time, which is desirable for realtime applications. To obtain a machine- and configuration-stable latency estimate, we perform a 1-job fMLLR decode on the full test set, record wall time, and divide by the total audio duration:
